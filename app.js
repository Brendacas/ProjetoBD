...
app.get('/payment', (req, res) => {
    res.render('payment', { numeroQuarto: req.query.numeroQuarto });
});

// Rota para finalizar reserva (apenas um exemplo)
app.post('/finalizar-reserva', async (req, res) => {
    const { numeroQuarto, tipoQuarto, preco, dataCheckIn, dataCheckOut, clienteEmail } = req.body;
  ...
  connection.query('INSERT INTO reserva SET ?', reserva, (err, result) => {
    if (err) {
    ...
    res.send('Reserva finalizada com sucesso');
  });
});
...
app.get('/historic', (req,res) => {
    ...
    connection.query(`SELECT quarto.NumeroQuarto, quarto.TipoQuarto, quarto.Preco, reserva.DataCheckIn, reserva.DataCheckOut
            ...
            if(err2) throw err2;
            console.log(total);
            res.render('historico', { compras: total }); //Renderiza a pagina historico.ejs
        });
})
